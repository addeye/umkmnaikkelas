...
class EventController extends Controller {
	...
	public function event_all() {
		...
		if (Auth::user()->role_id == ROLE_PENDAMPING) {
			$event = Event::whereIn('role_level', ['Semua', 'Pendamping'])->get();
		} else {
			$event = Event::whereIn('role_level', ['Semua', 'Umkm'])->get();
		}
	...
	public function event_follower(Request $request, $id) {
		...
		if ($event) {
			...
			return redirect()->route('event.show_akun_umkm', ['id' => $id]);
		}
	}
	...
	public function diskusi(Request $request) {
		...
		if ($diskusi) {
			...
			return redirect()->route('event.show_akun_umkm', ['id' => $request->event_id]);
		}
